...
export default async function CompanyLetsLeadsPage({ searchParams }: LeadsPageProps) {
  if (!getDatabaseConfig(appEnv).configured) {
    return (
      <AppShell>
        <EmptyState
          title="Company Lets lead room unavailable"
          description="Configure DATABASE_URL to load supply and demand company-let lead views."
        />
      </AppShell>
    );
  }
  ...
  try {
    ...
    console.error("Failed to load company lets lead room data.", error);
    leadService = null;
  ...
  return (
    <AppShell>
      <div className="space-y-8">
        <PageHeader
          eyebrow="Company Lets Leads"
          title="Supply and demand pipeline"
          description="Direct-only company lets board: verified stock and demand leads for residential operators and landlords."
        />
          ...
          <Card title="Lead room status">
            <div className="space-y-2">
              <p className="text-sm text-amber-300">
                The Company Lets lead board is temporarily unavailable while live data catches up.
              </p>
              <p className="text-xs pq-copy-subtle">
                Once the production schema is aligned, this board will recover automatically.
              </p>
            </div>
          </Card>
        ) : null}
          ...
          <div className="space-y-4">
            <div className="flex flex-wrap gap-2">
              {views.map((view) => (
                <Link
                  ...
                  href={`/internal/company-lets/leads?view=${view.value}${search ? `&search=${encodeURIComponent(search)}` : ""}`}
                  key={view.value}
                >
                  {view.label}
                </Link>
            ...
            >
              ...
              <input
                ...
                defaultValue={search}
                name="search"
                placeholder="Search company-let lead identity"
              />
              ...
              <Link
                ...
                href={`/internal/company-lets/leads?view=${selectedView}&page=1&pageSize=${pageSize}`}
              >
                Reset
              </Link>
            </form>
        ...
        <section className="grid gap-4 xl:grid-cols-[1.5fr_1fr]">
          <Card
            title={`${views.find((view) => view.value === selectedView)?.label} (${leads.length})`}
          >
            {leads.length === 0 ? (
              <EmptyState
                title="No company-let leads"
                description="No lead records currently match this view and search query."
              />
            ) : (
                  ...
                  >
                    <div className="flex flex-wrap items-center justify-between gap-3">
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <Badge tone="neutral">
                          Directness: {lead.directnessClassification}
                        </Badge>
                        <StatusPill
                          tone={
                    ...
                    <p className="mt-2 text-xs pq-copy-muted">
                      Last signal: {formatWhen(lead.lastSignalAt)}
                    </p>
                    <div className="mt-3 flex flex-wrap gap-3">
                      <Link
                        className="inline-flex min-h-11 items-center rounded-[var(--pq-radius-sm)] border border-[color:var(--pq-border)] px-4 text-sm text-white"
                        href={`/internal/company-lets/leads?view=${selectedView}&lead=${lead.id}&page=${page}&pageSize=${pageSize}${search ? `&search=${encodeURIComponent(search)}` : ""}`}
                      >
                        Open drawer
                      </Link>
                      {lead.leadType === "demand" ? (
                        <form action={extractDirectDemandAction}>
                          ...
                          <Button type="submit" variant="secondary">
                            Extract direct requirement
                          </Button>
                        </form>
                      ) : null}
                      <form
                        action={transitionLeadStatusAction}
            ...
            <div className="mt-4 flex items-center justify-between gap-3 border-t border-[color:var(--pq-border)] pt-3">
              <div className="text-xs pq-copy-subtle">
              ...
              <div className="flex gap-2">
                {page > 1 ? (
                  <Link
                    className="inline-flex min-h-11 items-center rounded-[var(--pq-radius-sm)] border border-[color:var(--pq-border)] px-4 text-sm text-white"
                    href={`/internal/company-lets/leads?view=${selectedView}&page=${page - 1}&pageSize=${pageSize}${search ? `&search=${encodeURIComponent(search)}` : ""}`}
                  >
                    Previous
                  </Link>
                ) : (
            ...
            {drawer ? (
              <div className="space-y-4 text-sm">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <Badge tone="neutral">
                    Directness: {drawer.directnessClassification}
                  </Badge>
                  <Badge tone="neutral">
                    Score version: {drawer.scoreVersion ?? "unscored"}
                  </Badge>
                  <StatusPill
                    tone={drawer.status === "qualified" ? "success" : "neutral"}
                  >
                    {drawer.status}
                  </StatusPill>
                </div>
                <dl className="grid gap-2">
                  <div className="flex items-center justify-between gap-3">
                    <dt className="pq-copy-subtle">Linked company</dt>
                    <dd>{drawer.companyName ?? "Unlinked"}</dd>
                  </div>
                  <div className="flex items-center justify-between gap-3">
                    <dt className="pq-copy-subtle">Linked contact</dt>
                    <dd>{drawer.contactName ?? "Unlinked"}</dd>
                  </div>
                  <div className="flex items-center justify-between gap-3">
                    <dt className="pq-copy-subtle">Linked property</dt>
                    <dd>{drawer.propertyTitle ?? "Unlinked"}</dd>
                  </div>
                </dl>
                ...
                <Card title="Directness verification gate">
                  <p className="pq-copy-muted text-sm">
                    Automated outreach requires DIRECT classification, verified status, and supporting evidence.
                  </p>
                  <form action={assessDirectnessAction} className="mt-3 grid gap-3">
                    <input name="leadId" type="hidden" value={drawer.id} />
                    <label className="space-y-1">
                      <span className="text-xs pq-copy-subtle">Entity</span>
                      <input
                        className="min-h-11 rounded-[var(--pq-radius-sm)] border border-[color:var(--pq-border)] bg-black/20 px-3 text-white"
                        name="entityName"
                        required
                    ...
                    <label className="space-y-1">
                      <span className="text-xs pq-copy-subtle">Person (optional)</span>
                      <input
                        className="min-h-11 rounded-[var(--pq-radius-sm)] border border-[color:var(--pq-border)] bg-black/20 px-3 text-white"
                        name="personName"
                      />
                    ...
                    <label className="space-y-1">
                      <span className="text-xs pq-copy-subtle">Relationship to property/company</span>
                      <input
                        className="min-h-11 rounded-[var(--pq-radius-sm)] border border-[color:var(--pq-border)] bg-black/20 px-3 text-white"
                        name="relationshipToPropertyOrCompany"
                        required
                      />
                    </label>
                    <label className="space-y-1">
                      <span className="text-xs pq-copy-subtle">Evidence source</span>
                      <input
                        className="min-h-11 rounded-[var(--pq-radius-sm)] border border-[color:var(--pq-border)] bg-black/20 px-3 text-white"
                        name="evidenceSource"
                        required
                    ...
                    <label className="space-y-1">
                      <span className="text-xs pq-copy-subtle">Evidence reference</span>
                      <input
                        className="min-h-11 rounded-[var(--pq-radius-sm)] border border-[color:var(--pq-border)] bg-black/20 px-3 text-white"
                    ...
                    <div className="grid gap-3 md:grid-cols-3">
                      <label className="space-y-1">
                        <span className="text-xs pq-copy-subtle">Evidence type</span>
                        <input
                          className="min-h-11 rounded-[var(--pq-radius-sm)] border border-[color:var(--pq-border)] bg-black/20 px-3 text-white"
                          defaultValue="ownership_record"
                          name="evidenceType"
                          required
                        />
                      </label>
                      <label className="space-y-1">
                        <span className="text-xs pq-copy-subtle">Evidence date</span>
                        <input
                          className="min-h-11 rounded-[var(--pq-radius-sm)] border border-[color:var(--pq-border)] bg-black/20 px-3 text-white"
                          name="evidenceDate"
                          type="date"
                      ...
                      <label className="space-y-1">
                        <span className="text-xs pq-copy-subtle">Confidence</span>
                        <input
                          className="min-h-11 rounded-[var(--pq-radius-sm)] border border-[color:var(--pq-border)] bg-black/20 px-3 text-white"
                          defaultValue="75"
                          max="100"
                    ...
                    <label className="space-y-1">
                      <span className="text-xs pq-copy-subtle">Proposed classification</span>
                      <select
                        className="min-h-11 rounded-[var(--pq-radius-sm)] border border-[color:var(--pq-border)] bg-black/20 px-3 text-white"
                    ...
                    <label className="space-y-1">
                      <span className="text-xs pq-copy-subtle">Explanation</span>
                      <textarea
                ...
                <Card title="WHY IS THIS LEAD DIRECT?">
                  {directnessAssessments.length === 0 ? (
                    <p className="pq-copy-muted">
                      No directness assessments recorded yet. Add evidence before any automated outreach.
                    </p>
                  ) : (
                    <div className="space-y-3">
                      {directnessAssessments.map((assessment) => (
                        ...
                        >
                          ...
                          <p className="mt-1 text-xs pq-copy-subtle">
                            Entity: {assessment.entityName}
                            {assessment.personName ? ` | Person: ${assessment.personName}` : ""}
                            {assessment.roleTitle ? ` | Role: ${assessment.roleTitle}` : ""}
                          </p>
                          <p className="mt-1 text-xs pq-copy-subtle">
                            Relationship: {assessment.relationshipToPropertyOrCompany}
                          </p>
                          <p className="mt-1 text-xs pq-copy-subtle">
                            Source: {assessment.evidenceSource} | Ref: {assessment.evidenceReference}
                          </p>
                        </article>
                      ))}
                    </div>
                ...
                <Card title="AI explanation placeholder">
                  {drawer.qualificationGuard.canUseForHighConfidenceQualification ? (
                    <p className="pq-copy-muted">
                      High-confidence qualification can use supported AI conclusions ({drawer.qualificationGuard.supportedConclusionCount}).
                    </p>
                  ) : (
                    <p className="pq-copy-muted">
                      No supported AI conclusion is currently eligible for high-confidence qualification.
                    </p>
                  )}
                </Card>
                <Card title="Action plan">
                  <form action={updateLeadPlanAction} className="space-y-3">
                    <input name="leadId" type="hidden" value={drawer.id} />
                    <label className="space-y-1">
                      <span className="text-xs pq-copy-subtle">Plan</span>
                      <textarea
                        className="min-h-24 rounded-[var(--pq-radius-sm)] border border-[color:var(--pq-border)] bg-black/20 px-3 py-2 text-white"
                        defaultValue={drawer.plan ?? ""}
                        name="plan"
                      />
                    </label>
                    <Button type="submit" variant="secondary">
              ...
              <EmptyState
                ...
                description="Choose a lead from the list to review the identity, provenance, and directness checks."
              />
            )}
          </Card>
        </section>
