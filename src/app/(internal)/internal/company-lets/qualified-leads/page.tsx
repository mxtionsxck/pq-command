...
export default async function CompanyLetsQualifiedLeadsPage({ searchParams }: LeadsPageProps) {
  if (!getDatabaseConfig(appEnv).configured) {
    return (
      <AppShell>
        <EmptyState
          title="Qualified leads unavailable"
          description="Configure DATABASE_URL to load the production Company Lets supply and demand lead workflow."
        />
      </AppShell>
    );
  }
  ...
  try {
    ...
    console.error("Failed to load Company Lets qualified leads.", error);
    leadService = null;
    ...
    <AppShell>
      <div className="space-y-8">
        <PageHeader
          ...
          description="Production direct-only lead board for residential stock and company-let demand."
        />
          ...
          <div className="space-y-4">
            <div className="flex flex-wrap gap-2">
              {views.map((view) => (
                <Link
                  ...
                  href={`/internal/company-lets/qualified-leads?view=${view.value}${search ? `&search=${encodeURIComponent(search)}` : ""}`}
                  key={view.value}
                >
                  {view.label}
                </Link>
            ...
            <form className="grid gap-3 md:grid-cols-[1fr_auto_auto]" method="get">
              ...
              <Link
                ...
                href={`/internal/company-lets/qualified-leads?view=${selectedView}&page=1&pageSize=${pageSize}`}
              >
                Reset
              </Link>
            </form>
                ...
                {leads.map((lead) => (
                  <article className="rounded-[var(--pq-radius-md)] border border-[color:var(--pq-border)] p-4" key={lead.id}>
                    ...
                    <div className="mt-3 flex flex-wrap gap-3">
                      <Link
                        className="inline-flex min-h-11 items-center rounded-[var(--pq-radius-sm)] border border-[color:var(--pq-border)] px-4 text-sm text-white"
                        href={`/internal/company-lets/qualified-leads?view=${selectedView}&lead=${lead.id}&page=${page}&pageSize=${pageSize}${search ? `&search=${encodeURIComponent(search)}` : ""}`}
                      >
                        Open drawer
                      </Link>
            ...
            <div className="mt-4 flex items-center justify-between gap-3 border-t border-[color:var(--pq-border)] pt-3">
              ...
              <div className="flex gap-2">
                {page > 1 ? (
                  <Link
                    className="inline-flex min-h-11 items-center rounded-[var(--pq-radius-sm)] border border-[color:var(--pq-border)] px-4 text-sm text-white"
                    href={`/internal/company-lets/qualified-leads?view=${selectedView}&page=${page - 1}&pageSize=${pageSize}${search ? `&search=${encodeURIComponent(search)}` : ""}`}
                  >
                    Previous
                  </Link>
                ) : (
