...
import { assessDirectnessAction } from "./directness-actions";
...
export default async function LeadsPage({ searchParams }: LeadsPageProps) {
  if (!getDatabaseConfig(appEnv).configured) {
    return (
      <AppShell>
        <EmptyState
          title="Lead Room unavailable"
          description="Configure DATABASE_URL to load lead views and workflows."
        />
      </AppShell>
    );
  }
    ...
    <AppShell>
      <div className="space-y-8">
          ...
          <div className="space-y-4">
            <div className="flex flex-wrap gap-2">
              {views.map((view) => (
                <Link
                  ...
                  href={`/internal/leads?view=${view.value}${search ? `&search=${encodeURIComponent(search)}` : ""}`}
                  key={view.value}
                >
                  {view.label}
                </Link>
            ...
            >
              ...
              <input
                ...
                defaultValue={search}
                name="search"
                placeholder="Search lead identity"
              />
              ...
              <Link
                ...
                href={`/internal/leads?view=${selectedView}&page=1&pageSize=${pageSize}`}
              >
                Reset
              </Link>
            </form>
        ...
        <section className="grid gap-4 xl:grid-cols-[1.5fr_1fr]">
                ...
                {leads.map((lead) => (
                  ...
                  >
                    ...
                    <div className="mt-3 flex flex-wrap gap-3">
                      <Link
                        className="inline-flex min-h-11 items-center rounded-[var(--pq-radius-sm)] border border-[color:var(--pq-border)] px-4 text-sm text-white"
                        href={`/internal/leads?view=${selectedView}&lead=${lead.id}&page=${page}&pageSize=${pageSize}${search ? `&search=${encodeURIComponent(search)}` : ""}`}
                      >
                        Open drawer
                      </Link>
            ...
            <div className="mt-4 flex items-center justify-between gap-3 border-t border-[color:var(--pq-border)] pt-3">
              ...
              <div className="flex gap-2">
                {page > 1 ? (
                  <Link
                    className="inline-flex min-h-11 items-center rounded-[var(--pq-radius-sm)] border border-[color:var(--pq-border)] px-4 text-sm text-white"
                    href={`/internal/leads?view=${selectedView}&page=${page - 1}&pageSize=${pageSize}${search ? `&search=${encodeURIComponent(search)}` : ""}`}
                  >
                    Previous
                  </Link>
                ) : (
          ...
          <Card
            title="Lead drawer"
            eyebrow="Identity, provenance, and action plan"
          >
            {drawer ? (
              <div className="space-y-4 text-sm">
                ...
                <Card title="Directness verification gate">
                  <p className="pq-copy-muted text-sm">
                    Automated outreach requires DIRECT classification, verified
                ...
                <Card title="WHY IS THIS LEAD DIRECT?">
                  {directnessAssessments.length === 0 ? (
                    <p className="pq-copy-muted">
                      No directness assessments recorded yet. Add evidence before
                ...
                <Card title="AI explanation placeholder">
                  {drawer.qualificationGuard
                    .canUseForHighConfidenceQualification ? (
                    <p className="pq-copy-muted">
                      High-confidence qualification can use supported AI
                      conclusions (
                      {drawer.qualificationGuard.supportedConclusionCount}
                      ).
                    </p>
                  ) : (
                    <p className="pq-copy-muted">
                      No supported AI conclusion is currently eligible for
                      high-confidence qualification.
                    </p>
                  )}
                </Card>
                {drawer.leadType === "demand" ? (
                  <Card title="Direct demand discovery">
                    <p className="pq-copy-muted text-sm">
                      Run guarded extraction to capture explicit requirements
                      only when demand intent is evidence-backed.
                    </p>
                    <form action={extractDirectDemandAction} className="mt-3">
                      <input name="leadId" type="hidden" value={drawer.id} />
                      <Button type="submit" variant="secondary">
                        Run direct demand extraction
                      </Button>
                    </form>
                  </Card>
                ) : null}
                <Card title="Next action and outreach status">
                  <form action={updateLeadPlanAction} className="grid gap-3">
                    <input name="leadId" type="hidden" value={drawer.id} />
                    <label className="space-y-2">
                      ...
                      <textarea
                        className="min-h-24 rounded-[var(--pq-radius-sm)] border border-[color:var(--pq-border)] bg-black/20 px-3 py-2 text-white"
                        defaultValue={drawer.nextAction ?? ""}
                        name="nextAction"
                      />
                    </label>
                    <label className="space-y-2">
                      ...
                      <select
                        className="min-h-11 rounded-[var(--pq-radius-sm)] border border-[color:var(--pq-border)] bg-black/20 px-3 text-white"
                        defaultValue={drawer.outreachStatus}
                        name="outreachStatus"
                      >
                ...
                <Card title="Signal timeline">
                  {drawer.signals.length === 0 ? (
                    <EmptyState
                      ...
                      description="No signal records linked to this lead yet."
                    />
                  ) : (
                    <div className="space-y-3">
                      {drawer.signals.map((signal) => (
                        <article
                          className="rounded-[var(--pq-radius-sm)] border border-[color:var(--pq-border)] p-3"
                          key={signal.id}
                        >
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            <Badge tone="neutral">{signal.status}</Badge>
                          </div>
                          <p className="mt-1 text-xs pq-copy-muted">
                            {signal.detectedAt.toLocaleString("en-GB")}
                          </p>
                          <p className="mt-2 text-xs text-white">
                ...
                <Card title="Evidence timeline">
                  {drawer.evidence.length === 0 ? (
                    <EmptyState
                      ...
                      description="No provenance evidence has been captured for this lead yet."
                    />
                  ) : (
              ...
              <EmptyState
                title="Drawer closed"
                description="Select a lead from the list to open the lead drawer."
              />
            )}
          </Card>
        </section>
